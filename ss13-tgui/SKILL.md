---
name: "ss13-tgui"
description: "Use when reviewing, implementing, simplifying, or debugging SS13 TGUI interfaces in BYOND/DM plus React (tgui-core or in-tree tgui) codebases (Vanderlin, /tg/, cmss13, BandaStation, Paradise, Goonstation and forks). Triggers on ui_interact/tgui_interact, ui_data/ui_static_data/ui_assets/ui_act, SStgui.try_update_ui/update_uis, tgui components (Button, Box, Section, Stack, Window), tgui SCSS/theming, TGUI performance or first-load delay, duplicate windows, and refactoring overcomplicated TGUI code. Reach for it whenever a task touches a tgui interface even if the user does not say the word TGUI."
---
...
Pick the closest authority: **same repo > same TGUI generation (tgui-core vs in-tree) > /tg/ canonical > external guides.** A neighbor in the current repo wins even over /tg/. External guides (below) are version-sensitive background, never the tiebreaker.
...
- `references/case-study-vanderlin-6578.md` — distilled, reusable lessons from PR #6578 and the Great Developer's review.
- `references/vanderlin-6578-timeline.md` — commit-by-commit timeline correlating mistakes, feedback, and corrected patterns.
...
Always prefer the conventions in the codebase you are working in over any external guide.
