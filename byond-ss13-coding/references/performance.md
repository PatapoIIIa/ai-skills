...
- SendMaps cost scales with clients × icons in view [dev 2023-12-14], and it is not hypothetical: busy SS13 servers have measured ~70% of the tick going to maptick, scaling linearly with player count, *after* their proc code was already optimized [dev 2023-04-03]. An experienced SS13 developer's account of the pre-BYOND-515 era: SendMaps left only **~20 ms** of tick budget to run all game logic with 60–80 concurrent players (some servers ran 250+), which is why that era's SS13 code was so aggressively hand-optimized — one server even considered splitting the simulation across two processes communicating via `world/Export()` to work around it [community 2025-12-11]. Modern multi-threaded SendMaps (see below) changes this balance, but the *shape* of the cost (scales with clients × view contents) is unchanged. Keeping perpetually-animating or perpetually-changing atoms out of dense areas is a real lever. **[measure]**
- Classic overlays are efficient *when they rarely change* — the cost is churn (each change re-reconciles the appearance and notifies clients), not existence [dev 2022-05-18]. Client-side, every `filter` on an object adds render passes each frame (a blur alone is two passes) — filters on many simultaneously-visible objects are a client framerate lever, invisible to server profiling [dev 2025-11-21].
