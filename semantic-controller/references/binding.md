...
1. **Classify the task domain.** TGUI/web UI → `ss13-tgui`. Fork content that must stay mergeable → `tgstation-modular-content`. Both can apply (a modular tgui interface) — their interaction order is defined in `run-skill-generator`. If no architecture skill covers the domain, work from the base alone; do not stretch a skill past its activation guard.
