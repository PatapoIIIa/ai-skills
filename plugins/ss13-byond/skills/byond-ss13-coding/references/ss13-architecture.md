...
- **`do_after` is a blocking polling loop, not a callback — read it that way before using it.** On tg it is literally `while (world.time < endtime) { stoplag(1); … }` (`code/__HELPERS/mobs.dm`): it sleeps your proc **and its whole caller chain** for the full duration, so it belongs in an interaction proc and is a red flag inside `process()`/`fire()` or any signal handler. What it does on each poll is the other half people miss — it already re-checks that the user still exists, has not moved, still holds the same item, is not incapacitated, and that the target still exists and has not moved, plus any `extra_checks` callback, and it returns `FALSE` the moment one fails. **Do not re-implement those checks around it**; pass `timed_action_flags` (`IGNORE_USER_LOC_CHANGE`, `IGNORE_HELD_ITEM`, `IGNORE_INCAPACITATED`…) to *relax* them, and `interaction_key` + `max_interact_count` to stop one user stacking the same action. It still returns to a world that moved on, so the `QDELETED()` re-validation rule applies to everything it did not check for you.
