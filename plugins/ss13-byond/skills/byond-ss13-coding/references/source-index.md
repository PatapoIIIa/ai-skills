# Source Index

Where this skill's claims come from, what was verified, and how to consult the sources yourself. This skill runs on whatever machine installs it — never assume any particular local file layout. If a task needs a source below and its location isn't known, ask the user rather than guessing a path.

## Contents

- [Source hierarchy](#source-hierarchy)
- [Verification results](#verification-results)
- [Consulting the DM Reference](#consulting-the-dm-reference)
- [Consulting the Discord developer logs](#consulting-the-discord-developer-logs)
- [Repository checkouts](#repository-checkouts)

## Source hierarchy

When sources conflict, higher wins for its own domain:

1. **Official DM Reference** — engine semantics. A copy ships with every BYOND install, typically under `<BYOND install directory>\help\ref\info.html` on Windows (e.g. under `Program Files (x86)\BYOND\`, but confirm on the machine at hand rather than assuming). Online: `https://www.byond.com/docs/ref/` (note: automated fetching may be blocked; the local copy is often the more reliable path). A searchable mirror of the same official content exists at `https://ref.dm-lang.org/` (`spacestation13/dm-ref` — a scraper + Quartz site, built for BYOND 516.1685, not independent commentary) — same tier, useful when a fetchable/searchable copy is needed and the local file isn't accessible; cross-check its BYOND build number against the one in play before citing.
2. **BYOND forum** (`https://www.byond.com/forum/`) — developer clarifications; date and BYOND version matter.
3. **Live repo code** — ground truth for what a codebase actually does; outranks any doc *about* the codebase.
4. **tgstation `.github/guides/`** — normative for tg conventions (tier 3); descriptive only for other forks.
5. **Fork guides** (`AGENTS.md`, CONTRIBUTING.md, local standards) — normative for their repo (tier 5 > 4).
6. **BYOND Discord developer logs** — heuristic grade: engine-developer statements are strong hints about internals, not documentation. Confirm against the reference or reproducible code when a decision hangs on one.
7. **Semantic bases (`ai_navigation*`)** — navigation aids; regenerable, never authoritative over code.

Tag discipline used throughout the references: `[DM Ref]`, `[tg]`, `[dev YYYY-MM-DD]` (the engine developer, on Discord), `[community YYYY-MM-DD]` (another experienced Discord participant — corroborating, not authoritative on internals), `[heuristic]`, `[measure]`. Keep the tags when editing; never promote a `[dev]`/`[community]`/`[heuristic]` claim to untagged fact without a documented source. **Never attach a real username/handle to a claim** — the tag records the *role* (engine developer vs. community participant), not identity; see Consulting the Discord developer logs below for why.

## Verification results

Checked against the local official DM Reference during authoring (2026-07-15):

**Corrections to prior local material** (`ai_navigation_vanders_river/engine_limits.md` and siblings derived from the old Designer's Guide):

| Old claim | Verified reality | Ref section |
|---|---|---|
| Bitwise ops are 16-bit (max 65535) | **24-bit, max 16,777,215** | operators |
| Max view range 10 (21×21) | **~5000 tiles, roughly 70×70** | view / client.view |
| `sleep(0)`/`spawn(0)` "cost at least one tick" | **Not supported.** `sleep(0)` sleeps "as short a time as possible"; `spawn(0)` runs after currently-pending events — sub-tick resumption is possible | sleep, spawn |

**Confirmed by the Reference**: `for(x in list)` iterates a copy (except `world`); `walk_to`/`step_to` silently no-op beyond 2×`world.view` steps; `world.cpu` ≥100 = tick overrun; the GC reference list (vars, lists, tag, map, contents, vis_contents, running/sleeping procs, keyed mobs, attached images); circular references never GC'd; `world.contents` is not a reference; turfs/areas not GC'd; `del` clears obvious refs then searches all memory; `spawn` copies plain locals; `set waitfor` semantics; `sleep(-1)` backlog-check semantics.

**Left unverified / heuristic** (do not present as engine fact): `luminosity` 0–6 range; "GC pressure between ticks" phrasing; relative VM cost of built-ins vs DM loops (directionally supported by tg guidance and dev statements, but unmeasured here); all `[measure]`-tagged performance claims.

**Second pass against `ref.dm-lang.org` (2026-07-18)**, cross-checking this skill's most load-bearing tier-1 claims against the mirror: the 24-bit bitwise claim matches the source **verbatim** ("A and B must be between 0 and 2\*\*24 - 1, giving an effective width of 24 bits"); the garbage-collection reference list, circular-reference, `del()` search, and immortal-object claims all matched verbatim. This pass also surfaced genuinely new material not previously covered and folded into dm-language-and-runtime.md: `DM_VERSION`/`DM_BUILD` preprocessor macros (and their `world`/`client` runtime equivalents) as the actual mechanism behind this skill's existing "515+"/"516+" notes; gliding (`glide_size`, `LONG_GLIDE`, the `LEGACY_MOVEMENT_MODE` gotcha); `/particles` (the indexed-vector silent-no-op gotcha, `count`/`spawning` as the performance knobs); and pixel movement (`Enter()`/`Exit()` irregular firing, `pixel_x/y/z` not moving the bounding box) — flagged as fork-conditional since most SS13 forks stay tile-based. Checked but judged not worth adding: the browser skin-control param list (already curated more narrowly and more deeply in `ss13-tgui/references/byond-client-api.md`, which cites the same official reference).

**Third pass — against a live tgstation master checkout (2026-07-18).** Every path and normative claim this skill attributes to tg was grepped against the real repo, not recalled. **12 of 13 cited `code/...` paths resolved exactly** (`__HELPERS/_lists.dm`, `__DEFINES/_tick.dm`, `__DEFINES/MC.dm`, `__DEFINES/qdel.dm`, `__DEFINES/subsystems.dm`, `__DEFINES/dcs/declarations.dm`, `controllers/master.dm`, `controllers/subsystem/garbage.dm`, `controllers/subsystem/processing/processing.dm`, `datums/datum.dm`, `datums/components/_component.dm`, `game/atom/atoms_initializing_EXPENSIVE.dm`); the 13th (`code/__pragmas.dm`) is correctly attributed to cmss13/Vanderlin, not tg, and tg indeed has no `#pragma` under `code/`. Confirmed verbatim in-repo: the assoc-list overhead figure (STANDARDS.md — "24 bytes per item overhead (vs 8 for lists and most vars)"), the `walk_*` ban with its full `SSmove_manager` replacement table (STANDARDS.md §The BYOND walk procs), `::`-over-`initial()` (STYLE.md, using the same `/obj/item/fork` example this skill uses), `typecacheof` as a proc in `_lists.dm`, the LAZY macro family, `CHECK_TICK`/`TICK_CHECK`/`TICK_USAGE`, `PROC_REF`/`TYPE_PROC_REF`/`GLOBAL_PROC_REF` (in `code/__byond_version_compat.dm`), `SHOULD_CALL_PARENT` (`__DEFINES/spaceman_dmm.dm`), `qdel` as a tg-authored proc (`garbage.dm`), and `iconstate2appearance` (`controllers/subsystem/overlays.dm`). `seconds_per_tick` is confirmed dominant over `delta_time` (671 files vs 9). Corrections this pass produced: the tick order gains a preceding "`walk()`-sleeping procs resume first" step, and the MC-wakes-last property is a *consequence* of sleep timing rather than deliberate design (both from TICK_ORDER.md); SendMaps gained tg's own per-player cost figure.

**Fourth pass — an experienced SS13 engineer's review, adjudicated 2026-09-01.** A `[community]`-grade reviewer raised ten objections; each was checked against the official DM Reference, a live tgstation checkout, and the engine developer's own statements before anything was edited. **Seven held and are now folded in:** `static` and `global` are one modifier with a single storage slot (so a static cannot be overridden per subtype — which constrains the scratch-list pattern); `sleep` stops the caller chain while `spawn` lets the current proc keep running; `world.cpu`/`map_cpu` are running averages, not per-tick values, and `tick_usage` under 100 can still be overtime; blur pass count scales with radius rather than being a flat two; `/mutable_appearance` is the intended churn-avoidance mechanism, not a footnote; `do_after` is a blocking `while` + `stoplag(1)` loop that already re-checks user/target state; and on tg, `SSspatial_grid` beats re-scanning with `view()` — but only for the three categories it indexes (hearing, client mobs, atmos machines), which the reviewer did not scope.

**Three did not hold, and the skill was deliberately left unchanged:** (1) *“use lazylists ALWAYS”* — tg's own `_lists.dm` says they are “best used on hot types when making lists that are not always used” and warns you “probably should not be using these macros if your list is not a lazylist”; the existing “rarely-used” wording already matched upstream. His premise (an unused empty list costs more than `null`) is correct and is now quoted. (2) *“qdel calls Del anyway, just batched”* — false on the success path: `del()` is reached only through `SSgarbage.HardDelete()`, normal refcount collection never calls it, and tg counts hard deletes as a failure metric. (3) *“the view maximum is larger than 70×70”* — the Reference says “about 5000 tiles, or roughly 70x70” verbatim and caps view depth at 34; the real nuance, now recorded, is that the budget is an *area*, so a non-square view may exceed 70 on one axis.

**Follow-up on the blur point (2026-09-01).** Shown the developer's own message, the reviewer argued the *mechanism* rather than a formula: a bigger blur cannot be had by widening the kernel, because stretching a fixed sample count over a larger radius degrades the image — so the engine adds passes instead. That is consistent with the developer's “more if it's a large blur” and with standard practice, and it is now the recorded explanation. The same message supplied the framing that matters more than pass-counting: **the dominant cost is the number of added render contexts, not shader maths** (“those costs are low compared to the cost of changing up contexts”), with concrete sample counts — 19 per Gaussian pass, 9 for outline, 2 for alpha. The skill leads with contexts and keeps the “shortcuts for huge blur” caveat, so no linear cost formula is implied.

**On the `do_after` table row the reviewer flagged:** it was never in this skill — it lives in `ai_navigation_twilight/core_procs.md`, a *semantic base* (fork data, deliberately outside the plugin, so no subscriber receives it). His criticism held there all the same: the row recorded the rule (“safe in interaction procs; suspicious in process/fire”) without the cause, so a reader could not generalise it to, say, a signal handler. The row now names the mechanism and routes to the skill for depth. Worth remembering when triaging review feedback against this repo: **the bases are public but are not the skills**, and a finding in one does not imply a defect in the other.

One claim was **unverifiable and is not recorded as fact**: that `world.cpu` misreports by summing SendMaps threads. SendMaps is genuinely multi-threaded [dev], and `world.cpu` genuinely covers map-send work [DM Ref], but no source states the summing behaviour — so the skill says only what is documented (running average, covers procs + map send) and stops there. The episode is itself the argument for tagging: a fluent, mostly-right reviewer was wrong three times out of ten, and only checking told which three.

## Consulting the DM Reference

The local `info.html` is one large file with `<a name=...>` anchors. Effective queries (PowerShell/bash + grep):

- Find a section: `grep -o '<a name=[^>]*sleep[^>]*>' info.html`
- Extract a section: `awk '/<a name=\/proc\/sleep>/,/<a name=\/proc\/next-entry>/' info.html | sed 's/<[^>]*>//g'`
- Anchors follow `/proc/name`, `/statement/...` (`for` lives at `/proc/for/list`), `/world/var/name`, `/DM/topic` (e.g. `/DM/garbage`).

Always note the installed BYOND version when citing (the ref documents *that* version).

## Consulting the Discord developer logs

If the user has a local export of the official BYOND Discord (DiscordChatExporter JSON, one file per channel — dev-support, dev-chat, engine-insights, 516-migration, bugs-and-issues, dev-design, dev-showcase, and others are the ones known to carry engine content), ask them for its location rather than assuming a path — this is local, user-provided data, never something to hardcode or bake into the skill.

- **Highest-value channel**: `engine-insights` is the densest on undocumented internals (garbage-scan order, associative-list tree structure, string interning, SendMaps cost model, appearance behavior, flick internals, istype/locate costs, scheduler mechanics); `dev-support`/`dev-chat`/`bugs-and-issues`/`dev-design`/`516-migration` are higher-volume but lower-density.
- **Filter by role, not by identity.** The export's `author.name` field lets you isolate the engine developer's own messages from everyone else's — that authorship distinction is exactly what makes a statement `[dev]`-grade vs `[community]`-grade. Never surface, store, or write down the literal handle anywhere in the skill's output: the tag records the *role*, and a work product should never carry a third party's username. Do the identity filtering only transiently, in a throwaway script, while producing de-personalized, role-tagged notes.
- **Citation discipline**: `[dev YYYY-MM-DD]` for the engine developer's own statements (heuristic grade — see Source hierarchy above); `[community YYYY-MM-DD]` for another participant whose message content demonstrates real expertise (production experience, implementation-level detail, corroborated by other sources) — still not authoritative on engine internals; anything less clearly grounded is not citable at all, regardless of how many messages that participant has posted. Message *volume* is not a signal of expertise — a prolific poster can still be a beginner thinking out loud. Judge each message by its content, not its author's post count. Check message dates against the BYOND version in play (pre/post 515/516 changes especially).
- **Privacy, always**: read-only; never copy a username, handle, or any other personally-identifying detail into skill content, notes, or any other work product — extract only the technical claim itself, tagged by role and date. Never upload the logs anywhere.

## Repository checkouts

This skill's repository-profiles.md was authored by statically reading read-only checkouts of six named codebases: tgstation, BandaStation, cmss13, CEV-Eris, Twilight-Axis, Vanderlin. Where those checkouts live — if they exist at all — is entirely local to whatever machine and workspace this skill runs in; never assume a fixed layout or hardcode a path. If a semantic base (`ai_navigation/`) is available for the repo at hand, find it via the `byond-codemaster-controller` skill's discovery protocol rather than by name-guessing a folder.

## Machine-checkable half

The claims on this page that can be tested against a checkout are encoded in `../claims.yaml` and run by `scripts/verify_claims.py` in the skills repository (weekly, or every 10 commits). A claim reported as **DRIFT** there has not been disproved — its evidence has expired, and it needs re-reading against the version that is now in the tree before being quoted at anyone. Update `last_verified` in the YAML and the dates here in the same pass.
