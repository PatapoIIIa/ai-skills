...
- [Lists](#lists)
- [static and global vars](#static-and-global-vars)
...
- **`static` and `global` are the same modifier in DM** — not two scopes. The reference documents `global` as making a var “permanent and shared… **only one copy of the var is maintained**”, and the engine developer refers to `var/static/foo` as a global var; `#define static global` compiles and works. Prefer `static` in SS13 code (it is the tg-idiomatic spelling and reads less like “GLOB”), but do not reason about it as if it were C++ class-scoped storage. [DM Ref: global var, dev]
- **A static var therefore cannot be overridden per subtype.** Declaring `var/static/list/foo` on a parent and assigning it again on a child does not give the child its own copy — there is one storage slot for the whole type tree, and every subtype shares it. If you want per-subtype data, you want an ordinary var, a lookup keyed by `type`, or a GLOB list — not a static.
- Consequence for the scratch-list pattern (performance.md → Allocations): a `var/static/list/scratch` is shared by every instance **and every subtype**, so it is only safe while a single call owns it start-to-finish. Any sleep, any recursion, any second caller in the same tick, and two logical users are writing into one list.
...
- **Single-argument `istype(x)` checks against x's own declared type.** With a fully typed parameter (`/mob/living/carbon/human/attacker`), `istype(attacker)` is exactly `ishuman(attacker)` but stays correct if the declaration later changes — the DRY form when the check exists only to validate the declared type. Prefer the named `isX()` helper when the check is about a *different* type than the declaration, or when the local codebase overwhelmingly uses the named form.
- **Declared types are documentation, not enforcement.** DM never checks that a var or parameter actually holds its declared type; assigning a string to a `var/datum/job/x` compiles and runs, and the mismatch only surfaces when someone reads `x.type` (silently null) or calls a proc on it (runtime). Real example: `/datum/mind/var/datum/job/assigned_role` is declared as a job datum, but tg-derived `SSjob.AssignRole` assigns the **title string** (`player.mind.assigned_role = rank`). Any code that trusts the declaration and reads `.type` off it gets nothing, and every downstream lookup fails silently. **Never trust a declared type on data you did not write** — verify what the assigning call sites actually store, and resolve defensively (`istype(x, /datum/job)` → `istext(x)` → fallback). This is the inverse of the "type paths, not names" rule: the fix is not to prefer the typed var, it is to check what is really in it.
...
- **The distinction that matters: `sleep` stops your caller chain, `spawn` does not stop anything.** [DM Ref: sleep, spawn] `sleep` “pauses the current proc *and its callers*” — the proc copies itself, the copy wakes later, and the same happens all the way up the stack, so everything waiting on your return value waits too. `spawn` copies only the indented statement/block; **the current proc keeps running immediately** and the copy takes its turn later. That is the whole reason `spawn` exists: “the caller does not have to wait around for the spawned code to finish”. Mechanically `sleep` parks the entire call stack and resumes it outside-in [dev 2023-11-12]. A sleeping proc keeps `src` and all its locals alive (GC references). If `src` is deleted (hard delete), the engine replaces the scheduled proc's src and the proc ends immediately on wake [dev 2026-02-19]. If some *other* object your proc references is deleted, your local var becomes a dangling reference to a qdel'd object or null — **always re-validate references after any sleep or `do_after`**: `if (QDELETED(target)) return`.
...
- `world.tick_lag` (deciseconds per tick) / `world.fps` set simulation rate. `world.cpu` covers **both running procs and the map-send work**, and `world.map_cpu` is a *subset* of it, not a separate budget [DM Ref: world vars]. **Both are running averages, not per-tick readings** — tg's own TICK_ORDER.md says you cannot derive maptick for any particular tick from `map_cpu` for exactly this reason. Only `world.tick_usage` is “this tick”. Consequences worth knowing before you quote any of them in a diagnosis: `tick_usage + map_cpu` never adds up to a meaningful per-tick total (an instantaneous value plus an average); **`tick_usage` below 100 can still be overtime**, because the tick has to cover map sending and other engine work on top of your code; and SendMaps is multi-threaded since 515 [dev], so cpu accounting across threads is not a wall-clock figure. Treat `world.cpu` as a coarse trend line, never as evidence about one tick. [community 2025-06-04]
- Tick order in practice (from tg's `TICK_ORDER.md`, [tg] — re-read against tgstation master 2026-07-18): **procs sleeping via `walk()` resume first** (before everything else — one more reason MC-driven codebases ban the `walk_*` family) → normal sleeping procs resume in the order they slept, and the Master Controller wakes here — **last among sleepers in practice, but as a *consequence*, not by design**: the MC almost always sleeps exactly 1 tick while most other procs sleep less or were called synchronously from it, so they re-enter the wake queue earlier. That ordering is what lets the MC account for previously-resumed procs' cost and minimise overtime → control passes to BYOND internals → **SendMaps** computes and sends per-client view updates → **client verbs execute at the very end of the tick**. Consequence: expensive verbs are the most likely code to overrun the tick — keep verbs thin and defer real work to a subsystem. The same thin-enqueuer discipline applies to `Topic()` handlers: an SS13 contributor tracing glide stutter found incoming verb/Topic calls execute immediately at their processing point rather than waiting politely, so a heavy handler stutters everything else — validate, enqueue, return. [community 2026-06-21]
...
- Max view size is about 5000 **tiles**, i.e. roughly 70×70 [DM Ref: client.view var — corrects older material claiming a hard max of 10]. The binding limit is the *area*, not 70 per side: a view depth caps at 34 (→ 69×69), but the `"WIDTHxHEIGHT"` form spends the same ~5000-tile budget however you like, so a deliberately wide view can exceed 70 on one axis while staying legal.
