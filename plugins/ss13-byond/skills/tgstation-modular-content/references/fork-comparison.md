...
**Rarity warning, measured 2026-09-01.** The aggregator is the better design, but it is rare enough that you must never assume it is what you are looking at. Across every Vanderlin- and Roguetown-lineage checkout available for re-verification — four forks, three of them carrying `modular_*` roots by the handful — **not one used an aggregator**: all shipped a flat per-file `#include` list in the `.dme`, and `_module.dm` existed in none of them. A downstream fork of an aggregator codebase does not necessarily inherit the aggregator. Grep the `.dme` before writing an include, and read the section above as "how to build this if you are starting fresh", not as "how this fork probably works." Provenance for every claim on this page: `source-index.md`.
